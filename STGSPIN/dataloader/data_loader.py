...
class CreateDataset(data.Dataset):
    def __init__(self, opt):
        ...
        # self.edge_paths, self.edge_size = make_dataset(opt.edge_file)
        ...
        # if hasattr(opt, 'mask_label_file') and opt.mask_label_file != 'none':
        #     self.mask_label_paths, self.mask_label_size = make_dataset(opt.mask_label_file)
    ...
    def __getitem__(self, index):
        ...
        # (new) load mask_label
        # if hasattr(self, 'mask_label_paths'):
        #     mask_label = self.load_mask_label(index)
        # else:
        #     mask_label = None  # 可选
        # load edge
        #edge = self.load_edge(index)
        return {
            'img': img,
            'img_path': img_path,
            'mask': mask,
            #'edge': edge,
            #'mask_label': mask_label  # 新增字段
        }
    ...
    def load_edge(self, index):
        ImageFile.LOAD_TRUNCATED_IMAGES = True
        img_path = self.edge_paths[index % self.edge_size]
        img_pil = Image.open(img_path).convert('RGB')
        img = self.transform(img_pil)
        img_pil.close()
        return img
    def load_mask(self, img, index):
        ...
        if mask_type == 3:
            mask_index = index % self.mask_size  # 强制一一对应

            mask_pil = Image.open(self.mask_paths[mask_index]).convert('RGB')
            size = min(mask_pil.size)
