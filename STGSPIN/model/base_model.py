...
class BaseModel():
    ...
    def save_results(self, save_data, score=None, data_name='none'):
        print(f"[DEBUG] save_results() called for {data_name}")
        print(f"[DEBUG] save_data shape: {save_data.shape if save_data is not None else 'None'}")
        img_paths = self.get_image_paths()
        print(f"[DEBUG] image paths: {img_paths}")
